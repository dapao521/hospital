...
const keshi = ref();
keshi.value = store.keshi;

// 高亮设置
const activeIndex = ref();
// 获取到对应的hi
const xiaobiaodi = ref();
...
const onclick = (i) => {
  ...
  // 获取到对应的h1 进行点击跳转
  // console.log(xiaobiaodi.value[activeIndex.value - 1]);
  // web原生方法scrollIntoView
  xiaobiaodi.value[activeIndex.value - 1].scrollIntoView({
    //behavior 过度动画效果
    behavior: "smooth",
  });
};
...
const guahao = () => {
  ...
  if (localStorage.getItem("token")) {
    router.push({ path: "/hospitol/guahao", query: { ids: 1, names: "北京医院" } });
  } else { 
    store.kaiguan();
  }
...
<template>
  <div class="big">
    ...
    <div class="down">
      ...
      <div class="youbian">
        <h1>挂号规则</h1>
        <span>预约周期：10天 放好时间：08:00 停挂时间:18:00</span>
        <span>具体地址：北京市市区下去习程序</span>
        <p>
          规划路线：东垣曲路线比巴斯u波伏娃不了把法律把我户口本放把呼啦何方维护本李夫人身边的v好问路边v卢卡斯把v外来人员的机会报复微博富豪榜温柔速度回来方块被u说的微博翻了u人看不是否更何况近日表示问候不是更好入睡半个可如今是大部分如黄金时代发布v恶化人家都是白发恶化日记本史蒂芬霍金让不少地方
          无不而非关键时刻还被如愿收到就回复不饿我好叫人生的包袱饿啊如黄金时代部分缓解人生的包袱可如今是被v的回家看认识不到而湖北省价格会如叫苦不迭是否关乎阿胶让不少的和v看见波尔多回家而不及时的告别v和u人恐惧的方式把v机会
        </p>
        <span>忒好时间：就挣钱一个工作日15:00前</span>
        <h1>医院预约规则</h1>
        <span v-for="i in 10" :key="i"
          >1.西城区预约取号地点：西城区们正一楼挂号窗口取号</span
        >
      </div>
    </div>
  </div>
  ...
  <div class="keshi">
    ...
    <div class="right">
      <div class="xiaokeshi" v-for="i in keshi" :key="i">
        <h1 ref="xiaobiaodi">{{ i.names }}</h1>
        <ul>
          <li v-for="item in i.childred" :key="item" @click="guahao">{{ item.names }}</li>
        </ul>
      </div>
    </div>
  </div>
